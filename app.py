...
def load_artifacts():
    try:
        ...
        for base_path in possible_paths:
            assets_files = ["assets.joblib", "assets__1_.joblib"]
            for assets_file in assets_files:
                assets_path = os.path.join(base_path, assets_file)
                if os.path.exists(assets_path):
                    assets = joblib.load(assets_path)
                    st.success(f"✅ Assets carregados de: {assets_path}")
                    break
            if assets:
                break
        ...
        # Procurar modelo
        for base_path in possible_paths:
            for model_file in ["catboost_model.cbm", "catboost_model (3).cbm"]:
        ...
        if assets is None or model is None:
            st.error("❌ Erro: Arquivos não encontrados!")
            st.info("Arquivos procurados: assets.joblib, catboost_model.cbm")
...
def build_features(inputs: dict):
    ...
    row = {
        ...
        **{k: int(v) for k, v in zone_ohe.items()},
